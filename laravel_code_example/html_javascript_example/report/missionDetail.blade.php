...
@section('content')
    ...
    <div>
        <div class="layout-px-spacing">
            ...
            <div class="modal" id="assignDifferentEmployee" tabindex="-1" role="dialog">
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">Başkasına Ata</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <div class="row">
                                <div class="col-12">
                                    <label for="">Yeni Atanacak Personel</label>
                                    <select id="otherEmployee" class="form-control">
                                        <option value="0" disabled selected>Seçiniz..</option>
                                        @foreach($users as $user)
                                            <option {{$user->id == $mission->assigned_user_id ? 'selected' : ''}} value="{{$user->id}}">{{$user->firstname}} {{$user->lastname}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-primary" onclick="assignOther()">Ata</button>
                            <button type="button" class="btn btn-secondary" data-dismiss="modal">Kapat</button>
                        </div>
                    </div>
                </div>
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="row mt-5" style="width: 100%;padding-top: 8px;border-top: 1px solid grey">
                                                ...
                                                <div class="container mt-5">
                                                    <div class="row mt-9" style="width: 100%;padding-top: 8px;border-top: 1px solid grey">                                                        <div class="col-12 text-center mb-3">
                                                            <h5>İşlemler</h5>
                                                        </div>
                                                        @if($mission->status == 1 && $mission->is_notify == 0)
                                                            <div class="col-12 text-center mb-3">
                                                                <button onclick="openNotifyModal()" class="btn btn-primary">Bildirim Gönder</button>
                                                            </div>
                                                        @elseif($mission->status == 2 && $mission->is_notify == 0)
                                                            <div class="col-12 text-center mb-3">
                                                                <button onclick="openNotifyModal()" class="btn btn-primary">Asılsız Mesajı Ayarla</button>
                                                            </div>
                                                        @endif

                                                        @if($mission->status == 0)
                                                            <div class="col-12 text-center mb-3">
                                                                <button class="btn btn-info" onclick="openAssignOtherModal()">Başkasına Ata</button>
                                                            </div>
                                                        @endif

                                                        @if($mission->status == 0)
                                                            <div class="col-12 text-center">
                                                                <a href="{{ route('report.missionsSolveIndex', $mission->id) }}" class="btn btn-info">Görevi Çöz</a>
                                                                <a href="{{ route('report.missionsFakeIndex', $mission->id) }}" class="btn btn-info ml-3">Asılsızlığı Bildir</a>
                                                            </div>
                                                        @endif
                                                    </div>
                                                </div>
...
@section('js')
    ...
    <script>
        ...
        function openAssignOtherModal(){
            $('#assignDifferentEmployee').modal('toggle')
        }
        ...
        function assignOther(){
            Swal.fire({
                icon:'warning',
                title:'Emin Misiniz !',
                text:'Bu Görevi Başka Personele Atamak İstediğinize Emin Misiniz ?',
                ...
                if(res.isConfirmed){
                    let newEmployeeId = document.getElementById('otherEmployee').value;
                    $.ajax({
                        url:'{{route('report.rollbackMission')}}',
                        type:'POST',
                        data:{
                        ...
                        success:(response)=>{
                            Swal.fire({
                                ...
                                window.location = '{{route('report.missions')}}'
                            })
